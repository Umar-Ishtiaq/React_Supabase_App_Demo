...
function App() {
  ...
  const addTodo = async ()=> {
    ...
    .select()//this will fetch the new task that we just saved in the supabase db

    if(error) {
      console.error('Error adding new task.', error)
    } else if (Array.isArray(data)) {
      setTodo(prevTodos => [...prevTodos, ...data]) //prevtodos is the todos in that are already in the todos state
      setNewTask('')
    }
  }

  const updateTodo = async(id) => {
    //update todos set task = editingText where id = taskid
